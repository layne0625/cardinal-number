...
export const getCardinalDescription = (num: number): string => {
  ...
  if (num < 1000000) {
    return getThousands2Millons(num).join(' ');
  }
  const millons = Math.floor(num / 1000000);
  const rests = num % 1000000;
  let strList = getThousands2Millons(rests);
  if (millons > 0) {
    const millonsStrList = getThousands2Millons(millons, true);
    // 百万大于1时用millones
    strList = [...millonsStrList, millons === 1 ? 'millón' : 'millones', ...strList];
  }
  return strList.join(' ');
};